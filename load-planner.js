...
(function(root){
  ...
  function renderResults(){
    ...
    $('load-results').replaceChildren();if(!samples.length){$('load-results').append(node('p','load-empty','No grid-consumption readings exist for the selected meter and analysis period.'));return;}
    ...
    $('load-data-note').textContent=`${selected.length.toLocaleString()} 15-minute grid-consumption readings in the analysis period. This is measured average kW over each 15-minute interval, not instantaneous demand.`;
  }
  function render(){
