(function(root){
  ...
  let plans=[],records=[],meter='',editing=null,demo=false;
  ...
  function save(){try{localStorage.setItem(key,JSON.stringify({version:1,plans}));$('plan-status').textContent='Plans saved on this browser.';}catch{$('plan-status').textContent='Plans could not be saved. Changes are available in this tab only; any previous saved plans are unchanged.';}}
  ...
  function render(){
    ...
    const keys=last?PowerPlans.period(last,$('plan-period').value):[];
    const results=plans.map(p=>last?PowerPlans.compare(records,meter,keys,p):null);
    $('plan-context').textContent=last?`${demo?'Sample data · ':''}Meter ···${meter.slice(-6)} · ${month(keys[0])}–${month(keys.at(-1))} · Grid consumption only. This period is independent of the chart dates above.`:'Import usage or explore sample data to calculate bills. You can add plans now.';
  ...
  function init(){
    ...
    $('add-plan').onclick=()=>edit();$('close-plan').onclick=()=>$('plan-dialog').close();$('cancel-plan').onclick=()=>$('plan-dialog').close();$('close-bill').onclick=()=>$('bill-dialog').close();
